...
/**
 * Handwritten-style toggle for B/I/U/S: text-only using Bradley Hand,
 * with a gentle glass plate and a warmer overlay when active.
 * 
 * @author S1mplector
 */
public class HandStyleToggleButton extends JToggleButton {
    ...
    private Font resolveFont() {
        if (HAND_FONT.getFamily().equalsIgnoreCase("dialog")) {
            return getFont().deriveFont(Font.PLAIN, 18f);
        }
        return HAND_FONT;
    }
